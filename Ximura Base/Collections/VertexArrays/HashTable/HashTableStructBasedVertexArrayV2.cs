...
namespace Ximura.Collections
{
    ...
    public class HashTableStructBasedVertexArrayV2<T> : MultiLevelStructBasedVertexArray<T, BucketArray<T>>
    {
        ...
        internal struct StructBasedVertexWindowV2<T> : IVertexWindow<T>
        {
            ...
            public StructBasedVertexWindowV2(HashTableStructBasedVertexArrayV2<T> data, IEqualityComparer<T> eqComparer, int indexID, int hashID, T item)
            {
                mData = data;
                mHashID = hashID;
                mItem = item;
        ...
        {
            mBuckets = new BucketArray<T>[LevelMax];
        }
        ...
        protected override void BucketLevelExpand(int currentLevel, int newLevel)
        {
            lock (syncBucketExpand)
            {
                ...
                {
                    mBuckets[level] = new BucketArray<T>(BucketLevelCapacityCalculate(level));
                }
